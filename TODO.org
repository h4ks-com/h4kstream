...
- [ ] Implement frontend [0/4]
  - [x] Show current playlist
  - [x] Show current stream info (live/user)
  - [x] Show past shows under /archive search open each show

  - [x] Basic login and user hashed pws in db simple backend. (generate signin url)
  - [x] File upload
  - [x] Youtube url
  - [x] Get key for streaming manually

  - [-] Youtube search
  - [x] Start custom stream (microphone)
  - [-] https://strudel.cc/workshop/getting-started/

- [x] Route everything over caddy. Load balancer based on request processing blocked. Multiple backends.
- [x] Keep internal EPs internal only
- [x] Helm chart?

- [x] db missing in helm
- [x] mixxx cannot stream to this thing
- [x] Webrtc not really working outside of my connection
- [x] mixxx metadata not setting
- [ ] Cache songs. Create new table with file hash before audio was trimmed, optional url. We dont store metadtata, if the user override we use the override, but we just avoid duplicating files. We have a column referencing the trimmed version we already have. Both for fallback and user queue, so that is also a column.
- [ ] Transitions always playing even though playlist didn't change...
