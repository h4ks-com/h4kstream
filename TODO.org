...
- [ ] Archive and list shows from disk
...
- [ ] Check if for user mdp queues there is a limit in the endpoints for upload, limit to 30 min songs, dont even donwload nad just fail otherwise. for amins uploads there should be no limit. If file also make it possible to reject and also have a size limit. Also dont allow addign something already in playlist (next 5 or so?)
- [ ] Webhooks subscription time to live? So they cleanup after some time?
