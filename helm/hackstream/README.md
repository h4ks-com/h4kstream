...
# Install the chart
helm install hackstream . -f values-prod.yaml -n hackstream --create-namespace
```
...
| `ingress.host` | Hostname for the application | `hackstream.local` |
...
| `backend.image.repository` | Backend image repository | `mattfly/hackstream-backend` |
...
helm upgrade hackstream . -f values-prod.yaml -n hackstream

# Upgrade with new image tag
helm upgrade hackstream . --set backend.image.tag=v1.2.3 -n hackstream
```

## Uninstalling

```bash
helm uninstall hackstream -n hackstream
```

**Note**: PVCs are not automatically deleted. Delete manually if needed:

```bash
kubectl delete pvc -n hackstream -l app.kubernetes.io/instance=hackstream
```
...
```bash
kubectl get pod -n hackstream -l app.kubernetes.io/component=janus -o yaml | grep hostNetwork
```

If `hostNetwork: true`, the backend should automatically use `status.hostIP`. Verify:

```bash
kubectl logs -n hackstream -l app.kubernetes.io/component=backend | grep JANUS_HOST
```
...
```bash
# Check PVC
kubectl get pvc -n hackstream

# Check if mounted correctly
kubectl exec -n hackstream deploy/hackstream-mpd-user -- ls -la /music
```
...
```bash
kubectl logs -n hackstream -l app.kubernetes.io/component=liquidsoap -f
```

Verify MPD connectivity:

```bash
kubectl exec -n hackstream deploy/hackstream-liquidsoap -- nc -zv hackstream-mpd-user 6600
```
...
```bash
kubectl get ingressroute -n hackstream
kubectl describe ingressroute hackstream -n hackstream
```
...
helm install hackstream . -f values-dev.yaml -n hackstream-dev --create-namespace

# Port-forward for local access
kubectl port-forward -n hackstream-dev svc/hackstream-backend 8000:8000
```
...
- GitHub: https://github.com/mattfly/hackstream
