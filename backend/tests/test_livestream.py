...
async def test_generate_livestream_token():
    ...
    token, expires_at = generate_livestream_token(3600)
...
async def test_validate_and_reserve_slot_success(livestream_service):
    ...
    token, _ = generate_livestream_token(3600)
...
async def test_validate_and_reserve_slot_invalid_token(livestream_service):
    ...
    success, reason = await livestream_service.validate_and_reserve_slot("invalid_token", "192.168.1.1")
...
async def test_validate_and_reserve_slot_already_occupied(livestream_service, redis_client):
    ...
    token1, _ = generate_livestream_token(3600)
    token2, _ = generate_livestream_token(3600)

    success1, _ = await livestream_service.validate_and_reserve_slot(token1, "192.168.1.1")
    assert success1 is True

    success2, reason2 = await livestream_service.validate_and_reserve_slot(token2, "192.168.1.2")
...
async def test_validate_and_reserve_slot_time_limit_exceeded(livestream_service, redis_client):
    ...
    token, _ = generate_livestream_token(100)
    payload = jwt.decode(token, options={"verify_signature": False})
...
async def test_time_limit_enforcement_logic(livestream_service, redis_client):
    ...
    token, _ = generate_livestream_token(5)
    payload = jwt.decode(token, options={"verify_signature": False})
    user_id = payload["user_id"]
