...
from tests.api_endpoints import QUEUE_LIST
from tests.api_endpoints import Playlist
from tests.api_endpoints import admin_queue_clear
from tests.api_endpoints import admin_queue_delete
from tests.api_endpoints import admin_queue_list
from tests.api_endpoints import queue_delete
...
def clean_queues(client: httpx.Client, admin_headers: dict[str, str]) -> None:
    ...
    client.post(admin_queue_clear(Playlist.USER), headers=admin_headers)
    client.post(admin_queue_clear(Playlist.FALLBACK), headers=admin_headers)
...
def test_token_with_default_max_queue_songs(client: httpx.Client, admin_headers: dict[str, str]) -> None:
    ...
    response = client.post(ADMIN_TOKEN_CREATE, json={"duration_seconds": 3600}, headers=admin_headers)
    assert response.status_code == 200
    assert "token" in response.json()
...
def test_fallback_list_endpoint_exists(client: httpx.Client, admin_headers: dict[str, str]) -> None:
    """Test that radio playlist list endpoint exists and returns empty list after clear."""
    client.post(admin_queue_clear(Playlist.FALLBACK), headers=admin_headers)
    response = client.get(admin_queue_list(Playlist.FALLBACK), headers=admin_headers)
    assert response.status_code == 200
...
def test_user_list_endpoint_public(client: httpx.Client, admin_headers: dict[str, str]) -> None:
    ...
    client.post(admin_queue_clear(Playlist.USER), headers=admin_headers)
    response = client.get(QUEUE_LIST)
    assert response.status_code == 200
    assert isinstance(response.json(), list)
    assert len(response.json()) == 0
...
def test_queue_clear_endpoints_work(client: httpx.Client, admin_headers: dict[str, str]) -> None:
    ...
    user_clear = client.post(admin_queue_clear(Playlist.USER), headers=admin_headers)
    assert user_clear.status_code == 200

    radio_clear = client.post(admin_queue_clear(Playlist.FALLBACK), headers=admin_headers)
    assert radio_clear.status_code == 200
...
def test_different_tokens_have_different_user_ids(client: httpx.Client, admin_headers: dict[str, str]) -> None:
    ...
    token1 = client.post(
        ADMIN_TOKEN_CREATE, json={"duration_seconds": 3600, "max_queue_songs": 1}, headers=admin_headers
    ).json()["token"]
...
def test_admin_endpoints_reject_jwt_tokens(client: httpx.Client, admin_headers: dict[str, str]) -> None:
    ...
    jwt_token = client.post(ADMIN_TOKEN_CREATE, json={"duration_seconds": 3600}, headers=admin_headers).json()["token"]
...
def test_max_add_requests_enforced_with_deletes(client: httpx.Client, admin_headers: dict[str, str]) -> None:
    ...
    response1 = client.post(QUEUE_ADD, data={"url": test_url}, headers=jwt_headers, timeout=60.0)
    ...
    queue = client.get(QUEUE_LIST).json()
    assert len(queue) == 1
    song_id_1 = queue[0]["id"]
    delete_response1 = client.delete(queue_delete(song_id_1), headers=jwt_headers)
    assert delete_response1.status_code == 200

    # Verify queue is empty after delete
    queue_after_delete = client.get(QUEUE_LIST).json()
    ...
    response2 = client.post(QUEUE_ADD, data={"url": test_url}, headers=jwt_headers, timeout=60.0)
    assert response2.status_code == 200, f"Second add failed: {response2.json()}"

    # Delete song 2
    queue = client.get(QUEUE_LIST).json()
    song_id_2 = queue[0]["id"]
    delete_response2 = client.delete(queue_delete(song_id_2), headers=jwt_headers)
    assert delete_response2.status_code == 200

    # Add song 3 (should succeed - add count is 3, hitting the limit)
    response3 = client.post(QUEUE_ADD, data={"url": test_url}, headers=jwt_headers, timeout=60.0)
    ...
    delete_response3 = client.delete(queue_delete(song_id_3), headers=jwt_headers)
    assert delete_response3.status_code == 200

    # Try to add song 4 (should FAIL - add count would be 4, exceeding limit of 3)
    response4 = client.post(QUEUE_ADD, data={"url": test_url}, headers=jwt_headers, timeout=60.0)
