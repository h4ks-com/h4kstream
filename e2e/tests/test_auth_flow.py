...
def test_admin_token_auth(client: httpx.Client, admin_headers: dict[str, str]) -> None:
    ...
    response = client.post(admin_queue_clear(Playlist.USER), headers=admin_headers)
...
def test_admin_token_auth_fails_with_invalid_token(client: httpx.Client) -> None:
    ...
    response = client.post(admin_queue_clear(Playlist.USER), headers=headers)
...
def test_jwt_token_validation(client: httpx.Client, admin_headers: dict[str, str]) -> None:
    ...
    token_response = client.post(ADMIN_TOKEN_CREATE, json={"duration_seconds": 3600}, headers=admin_headers)
...
def test_full_auth_flow(client: httpx.Client, admin_headers: dict[str, str]) -> None:
    ...
    clear_response = client.post(admin_queue_clear(Playlist.USER), headers=admin_headers)
    assert clear_response.status_code == 200

    list_response = client.get(QUEUE_LIST)
...
def test_jwt_max_duration_enforcement(client: httpx.Client, admin_headers: dict[str, str]) -> None:
    ...
    response = client.post(ADMIN_TOKEN_CREATE, json={"duration_seconds": 100000}, headers=admin_headers)
    assert response.status_code == 422

    valid_response = client.post(ADMIN_TOKEN_CREATE, json={"duration_seconds": 86400}, headers=admin_headers)
...
def test_admin_can_access_admin_endpoints(client: httpx.Client, admin_headers: dict[str, str]) -> None:
    ...
    token_response = client.post(ADMIN_TOKEN_CREATE, json={"duration_seconds": 1800}, headers=admin_headers)
    assert token_response.status_code == 200
